...
export default function CO2Badge({ routes }: CO2BadgeProps) {
  ...
  const parisNYFlights = Math.round(stats.co2SavedPerYear / 1.75);
  const carsEquiv = Math.round(stats.co2SavedPerYear / 2.3);
  const earthLaps = (stats.kmAvoidedPerYear / 40_075).toFixed(1);

  return (
    <div
      className="glass-panel rounded-md px-2 sm:px-3 py-1.5 sm:py-2 max-w-[180px] sm:max-w-[220px] cursor-pointer transition-all"
      onClick={() => setExpanded(!expanded)}
    >
      {/* Header */}
      <div className="flex items-center gap-1 sm:gap-1.5 mb-1">
        <svg width="10" height="10" viewBox="0 0 12 12" fill="none" className="flex-shrink-0 sm:w-3 sm:h-3">
        ...
        <span className="text-[9px] sm:text-[10px] font-semibold text-text">Bilan CO2</span>
        <svg
          width="8" height="8" viewBox="0 0 8 8" fill="none" className={`ml-auto transition-transform hidden sm:block ${expanded ? 'rotate-180' : ''}`}
        >
      ...
      <div className="sm:hidden">
        <div className="flex items-center justify-between">
          <span className="text-[8px] text-muted">CO2 évité/an</span>
          <span className="font-mono text-[10px] font-bold text-cyan">{formatCO2(stats.co2SavedPerYear)}t</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-[8px] text-muted">Camions évités</span>
          <span className="font-mono text-[10px] font-bold text-blue">{formatNum(stats.trucksAvoidedPerYear)}/an</span>
        </div>
      </div>
      ...
      {expanded && (
        <div className="hidden sm:block mt-2 pt-2 border-t border-border space-y-1.5">
      ...
      {/* Hint — desktop only */}
      {!expanded && (
        <div className="mt-1 text-[8px] text-muted/60 text-center hidden sm:block">
