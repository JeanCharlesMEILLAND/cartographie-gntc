...
export default function TimeControl({ trainCount }: TimeControlProps) {
  ...
  const handleLive = useCallback(() => {
    setClockDay(getCurrentDay());
    setClockTime(getCurrentTimeMinutes());
    setClockPlaying(false);
  }, [setClockDay, setClockTime, setClockPlaying]);
  ...
  return (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[1000] glass-panel rounded-lg px-4 py-2.5 flex items-center gap-3 max-w-[720px] w-[95%] sm:w-auto">
      ...
      <button
        ...
        title={clockPlaying ? 'Pause' : 'Lecture'}
      >
        {clockPlaying ? (
      ...
      <button
        onClick={handleLive}
        className="flex items-center gap-1 text-[10px] font-medium text-muted hover:text-blue transition-colors flex-shrink-0 px-1.5 py-0.5 rounded border border-border hover:border-blue/30"
